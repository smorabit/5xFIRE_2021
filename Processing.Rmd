...
# save the unprocessed & unfiltered seurat object:
saveRDS(seurat_obj, file='/dfs3b/swaruplab/smorabit/analysis/FIRE_mouse_2021/data/FIRE_mouse_unfiltered_unprocessed_seurat.rds')
...
comparisons <- list(c(2,4), c(1,3), c(6,2), c(6,1))
...
pdf(paste0(fig_dir, 'vlnplot_DAM_DX_with_transplant_homeostatic.pdf'), width=4, height=4)
for(p in plot_list){
  print(p)
}
dev.off()
...
pdf(paste0(fig_dir, 'vlnplot_DAM_DX_with_transplant_stage1.pdf'), width=4, height=4)
for(p in plot_list){
  print(p)
}
dev.off()
...
pdf(paste0(fig_dir, 'vlnplot_DAM_DX_with_transplant_stage2.pdf'), width=4, height=4)
for(p in plot_list){
  print(p)
}
dev.off()
