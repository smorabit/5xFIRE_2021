...
pdf(paste0(fig_dir, 'volcano.pdf'), width=10, height=10, useDingbats=FALSE)
...
for(cl in unique(seurat_obj$class)){
  print(cl)
  seurat_degs <- subset(seurat_obj, class == cl & DX %in% c('5xFIRE + PBS', '5xFIRE + transplant'))
  Idents(seurat_degs) <- seurat_degs$DX
  de_markers <- FindMarkers(
    seurat_degs,
...
for(cl in unique(seurat_obj$class)){
  print(cl)
  seurat_degs <- subset(seurat_obj, class == cl & DX %in% c('5xFIRE', '5xFIRE + transplant'))
  Idents(seurat_degs) <- seurat_degs$DX
  de_markers <- FindMarkers(
    seurat_degs,
    ident.1 = '5xFIRE + transplant',
...
for(cl in unique(seurat_obj$class)){
  print(cl)
  seurat_degs <- subset(seurat_obj, class == cl & DX %in% c('5xFAD', '5xFIRE + transplant'))
  Idents(seurat_degs) <- seurat_degs$DX
  de_markers <- FindMarkers(
    seurat_degs,
    ident.1 = '5xFIRE + transplant',
...
marker.genes.up <- markers %>% subset(group == 'IMM' ) %>% top_n(25, wt=avg_logFC) %>% .$gene
marker.genes.down <- markers %>% subset(group == 'IMM' ) %>% top_n(-25, wt=avg_logFC) %>% .$gene
...
p1 <- ggplot(plot_df, aes(x=Combined.Score, y=reorder(wrap, Combined.Score)))+
 ...
 ) + ylab('') + xlab('Combined.Score') + ggtitle('Upregulated in 5xFIRE-Transplant IMM')


plot_df <- cur_result_down$GO_Biological_Process_2018 %>% top_n(15, wt=Combined.Score)
plot_df$Term <- do.call(rbind, strsplit(plot_df$Term, '[(]GO:'))[,1]
plot_df$wrap <- wrapText(plot_df$Term, 35)
...
theme(
  ...
  panel.grid.minor=element_blank(),
  legend.title = element_blank()
) + ylab('') + xlab('Combined.Score') + ggtitle('Upregulated in 5xFAD IMM')


pdf(paste0('figures/5xFIRE-transplant_vs_5xFAD_IMM_GOterms.pdf'), width=5, height=10)
p1
p2
dev.off()
