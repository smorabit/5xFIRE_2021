...
# re-order cluster levels based on dendrogram:
# seurat_obj$dendrogram_clusters <- factor(
#   as.numeric(as.character(seurat_obj$leiden_res1.5)), levels=c(
#   32, 38, 29, 37, 12, 15, 23, 43, 1, 41, 40, 16, 19, 11, 0, 31, 30, 22, 6, 14, 10, 39, 33, 2, 28, 13, 8, 9, 26, 7, 42, 27, 35, 21, 17, 44, 25, 3, 18, 36, 20, 24, 5, 34, 4
# ))
#
# clusternum_anno_levels <- sapply(levels(seurat_obj$dendrogram_clusters), function(x){subset(seurat_obj@meta.data, dendrogram_clusters == x) %>% .$clusternum_anno %>% as.character %>% unique})
#
# seurat_obj$clusternum_anno <- factor(seurat_obj$clusternum_anno, levels=as.character(clusternum_anno_levels))
...
# metacell_seurat <- scWGCNA::metacells_by_groups(seurat_obj, k=50, group.by=c('class', 'DX'))
...
# cur_seurat <- FindVariableFeatures(cur_seurat, nfeatures=3500)
# VariableFeatures(cur_seurat) <- VariableFeatures(cur_seurat)[!grepl("^mt-", VariableFeatures(cur_seurat))]
#
# # how many genes overlap?
# length(intersect(VariableFeatures(cur_seurat), VariableFeatures(seurat_obj)))

# which genes to use?
genes.use <- VariableFeatures(seurat_obj)
# genes.use <- VariableFeatures(cur_seurat)

# genes that are in 25% of cells:
# expr_mat <- GetAssayData(cur_seurat, slot='counts')
# expr_mat[expr_mat>0] <- 1
# gene_filter <- rowSums(expr_mat) > round(0.25*ncol(cur_seurat)); table(gene_filter);
# genes.use <- rownames(seurat_obj)[gene_filter]
#
...
plot_df$DX <- factor(plot_df$DX, levels=rev(levels(seurat_obj$DX)))
...
#################################################################
# split module box plots by cluster
#################################################################

# END:
w = 4+2*(nmodules-1); h = 4 * length(unique(plot_df$class))
...
pdf(paste0(fig_dir, cur_celltype, '_ME_trajectory_Plot_DX_split_celltype.pdf'),width=w,height=h,useDingbats=F)
p + facet_wrap(class~variable, scales='free', ncol=nmodules-1) +
  stat_compare_means(method='wilcox', comparisons=comparisons, label='p.signif')
dev.off()

pdf(paste0(fig_dir, cur_celltype, '_ME_trajectory_Plot_DX_split_celltype_no_comparison.pdf'),width=w,height=5,useDingbats=F)
p + facet_wrap(class~variable, scales='free', ncol=nmodules-1)
dev.off()
...
# where are my favorite genes:
geneInfo[grepl('Pdgf', rownames(geneInfo)),]
geneInfo[grepl('Vegf', rownames(geneInfo)),]
geneInfo[grepl('Tgfb', rownames(geneInfo)),]
...
module_df <- subset(seurat_obj@meta.data, class == cur_celltype)

# all clusters:
module_df <- seurat_obj@meta.data; group <- 'all_clusters'; w = 18; h=6;
# module_df <- subset(seurat_obj@meta.data, class == cur_celltype); group <- paste0(cur_celltype, '_clusters'); w=6; h=6;
...
geneInfo[grep('Mobp', geneInfo$GeneSymbol),]
